...
APP_NAME = "Asset Scraper Studio v21"
USER_AGENT = "Mozilla/5.0 AssetScraperStudio/21.0"
...
IMAGE_EXTS = {".png", ".jpg", ".jpeg", ".gif", ".svg", ".webp", ".bmp", ".ico", ".tga", ".dds"}
SOUND_EXTS = {".mp3", ".ogg", ".wav", ".flac", ".aac", ".m4a", ".mid", ".midi"}
TEXT_EXTS = {".as", ".xml", ".json", ".txt", ".csv", ".js", ".css", ".html", ".htm", ".php", ".asp", ".aspx", ".jsp", ".cfg", ".ini", ".conf", ".properties", ".log", ".yaml", ".yml", ".scss", ".less", ".mjs", ".ts"}
HTML_EXTS = {"", ".html", ".htm", ".shtml", ".php", ".php3", ".php4", ".php5", ".asp", ".aspx", ".jsp", ".cgi"}
...
TR = {
    "de": {
        ...
        "about": """Asset Scraper Studio v21
...
- TXT/CSV/JSON Export
- ZIP-Export
- Download aller Treffer
- Download sichtbarer Treffer
- Schnell-Download nach Dateityp
- erweiterte Asset-Recovery-Endungsliste
- HTML/PHP/ASP/JSP-Erkennung
- Game-Asset-Formate wie ATLAS, BUNDLE, PACK, PKG
- Fonts, Videos, Dokumente und Archive
- Deutsch/Englisch
...
def type_folder_for_ext(ext):
    ...
    if ext in SOUND_EXTS:
        return "sounds"
    if ext in {".mp4", ".webm", ".avi", ".mkv", ".mov", ".flv"}:
        return "videos"
    if ext in {".ttf", ".otf", ".woff", ".woff2", ".eot"}:
        return "fonts"
    if ext in {".zip", ".rar", ".7z", ".tar", ".gz", ".bz2", ".xz", ".iso", ".cab", ".jar"}:
        return "archives"
    if ext in {".html", ".htm", ".shtml", ".php", ".php3", ".php4", ".php5", ".asp", ".aspx", ".jsp", ".cgi"}:
        return "html"
    if ext in {".xml", ".json", ".yaml", ".yml", ".ini", ".cfg", ".conf", ".properties", ".dat", ".bin", ".db", ".sqlite", ".sqlite3"}:
        return "data"
    if ext in {".as", ".js", ".mjs", ".ts", ".css", ".scss", ".less"}:
        return "code"
    if ext in {".swf", ".sol"}:
        return "flash"
    if ext in {".atlas", ".sprite", ".assets", ".bundle", ".pack", ".pkg", ".manifest", ".cache", ".cachemanifest"}:
        return "game_assets"
    if ext in {".txt", ".rtf", ".pdf", ".doc", ".docx", ".odt", ".csv", ".log"}:
        return "documents"
...
def find_urls(base_url, text):
    ...
    pattern = r"""["'(\s]([^"'()\s<>]+?\.(?:7z|aac|as|asp|aspx|atlas|assets|avi|bin|bmp|bundle|bz2|cache|cachemanifest|cab|cfg|cgi|conf|css|csv|dat|db|dds|doc|docx|eot|flac|flv|gif|gz|htm|html|ico|ini|iso|jar|jpeg|jpg|js|json|jsp|less|log|m4a|manifest|mid|midi|mjs|mkv|mov|mp3|mp4|odt|ogg|otf|pack|pdf|php|php3|php4|php5|pkg|png|properties|rar|rtf|scss|shtml|sol|sprite|sqlite|sqlite3|svg|swf|tar|tga|ts|ttf|txt|wav|webm|webp|woff|woff2|xml|xz|yaml|yml|zip)(?:\?[^"'()\s<>]*)?)"""
...
class AssetScraperApp:
    ...
    def build_ui(self):
        ...
        filetypes_tab = ttk.Frame(notebook)
        ...
        notebook.add(filetypes_tab, text=self.l("Dateitypen", "File types"))
        ...
        self.build_filetypes_tab(filetypes_tab)
    ...
    def build_scraper_tab(self, tab):
        ...
        ttk.Label(top, text="   v21 Layout + Dateitypen-Tab", style="Muted.TLabel").pack(side="left")
        ...
        filetype_info = self.lf(right, self.l("Dateitypen", "File types"))
        filetype_info.pack(fill="x", pady=6)
        ttk.Label(
            filetype_info,
            text=self.l(
                "Dateitypen sind jetzt im eigenen Tab. Dadurch bleibt die Hauptansicht kompakt.",
                "File types are now in their own tab. This keeps the main view compact."
            )
        ...
        ttk.Button(buttons, text="⬇ Auswahl", command=self.start_download).pack(side="left", padx=3)
        ttk.Button(buttons, text="⬇ Alle", command=self.download_all_results).pack(side="left", padx=3)
        ttk.Button(buttons, text="⬇ Sichtbare", command=self.download_visible_results).pack(side="left", padx=3)
        ttk.Button(buttons, text="☑ Alle markieren", command=self.select_all_results).pack(side="left", padx=3)
        ttk.Button(buttons, text="☐ Aufheben", command=self.clear_selection).pack(side="left", padx=3)
        ...
        quick_downloads = ttk.Frame(right)
        quick_downloads.pack(fill="x", pady=(0, 4))
        ttk.Label(quick_downloads, text="Schnell-Download:").pack(side="left", padx=4)
        ttk.Button(quick_downloads, text="Flash", command=lambda: self.download_by_type("flash")).pack(side="left", padx=3)
        ttk.Button(quick_downloads, text="HTML", command=lambda: self.download_by_type("html")).pack(side="left", padx=3)
        ttk.Button(quick_downloads, text="Code", command=lambda: self.download_by_type("code")).pack(side="left", padx=3)
        ttk.Button(quick_downloads, text="Daten", command=lambda: self.download_by_type("data")).pack(side="left", padx=3)
        ttk.Button(quick_downloads, text="Bilder", command=lambda: self.download_by_type("images")).pack(side="left", padx=3)
        ttk.Button(quick_downloads, text="Sounds", command=lambda: self.download_by_type("sounds")).pack(side="left", padx=3)
        ttk.Button(quick_downloads, text="Videos", command=lambda: self.download_by_type("videos")).pack(side="left", padx=3)
        ttk.Button(quick_downloads, text="Archive", command=lambda: self.download_by_type("archives")).pack(side="left", padx=3)
        ttk.Button(quick_downloads, text="GameAssets", command=lambda: self.download_by_type("game_assets")).pack(side="left", padx=3)
        ...
        self.filter_values = ["All", "flash", "html", "code", "data", "images", "sounds", "videos", "fonts", "archives", "documents", "game_assets", "favorite", "saved", "found", "Error", "Duplicates"]
    ...
    def build_filetypes_tab(self, tab):
        wrapper = ttk.Frame(tab, padding=12)
        wrapper.pack(fill="both", expand=True)

        header = ttk.Frame(wrapper, style="Panel.TFrame", padding=10)
        header.pack(fill="x", pady=(0, 10))
        ttk.Label(header, text=self.l("Dateitypen", "File types"), style="Title.TLabel").pack(side="left")
        ttk.Label(
            header,
            text=self.l(f"   {len(DEFAULT_EXTENSIONS)} Endungen verfügbar", f"   {len(DEFAULT_EXTENSIONS)} extensions available"),
            style="Muted.TLabel"
        ).pack(side="left", padx=8)

        btns = ttk.Frame(wrapper)
        btns.pack(fill="x", pady=(0, 8))
        ttk.Button(btns, text=self.l("Alle aktivieren", "Enable all"), command=self.enable_all_extensions, style="Accent.TButton").pack(side="left", padx=4)
        ttk.Button(btns, text=self.l("Alle deaktivieren", "Disable all"), command=self.disable_all_extensions).pack(side="left", padx=4)
        ttk.Button(btns, text=self.l("Browsergame Standard", "Browser game default"), command=self.enable_browsergame_extensions).pack(side="left", padx=4)
        ttk.Button(btns, text=self.l("Nur Web/HTML", "Only Web/HTML"), command=self.enable_web_extensions).pack(side="left", padx=4)
        ttk.Button(btns, text=self.l("Nur Medien", "Only media"), command=self.enable_media_extensions).pack(side="left", padx=4)

        outer = ttk.Frame(wrapper)
        ...
        for i, ext in enumerate(sorted(set(DEFAULT_EXTENSIONS), key=lambda x: x.lower())):
            ...
            ttk.Checkbutton(scroll_frame, text=ext, variable=var).grid(
                row=i // columns,
                column=i % columns,
                sticky="w",
                padx=14,
        ...
        ttk.Label(
            wrapper,
            text=self.l(
                "Hinweis: Bei sehr großen Suchläufen nur benötigte Typen aktivieren.",
                "Tip: For very large searches, enable only the required file types."
            ),
    ...
    def enable_all_extensions(self):
        for var in self.ext_vars.values():
            var.set(True)
        self.log("Alle Dateitypen aktiviert.")

    def disable_all_extensions(self):
        for var in self.ext_vars.values():
            var.set(False)
        self.log("Alle Dateitypen deaktiviert.")

    def enable_browsergame_extensions(self):
        wanted = {
            ".swf", ".as", ".sol", ".xml", ".json", ".yaml", ".yml",
            ".js", ".mjs", ".css", ".png", ".jpg", ".jpeg", ".gif", ".svg", ".webp", ".ico",
            ".mp3", ".ogg", ".wav", ".bin", ".dat", ".cfg", ".ini", ".properties",
            ".atlas", ".sprite", ".assets", ".bundle", ".pack", ".pkg", ".zip", ".rar", ".7z"
        }
        for ext, var in self.ext_vars.items():
            var.set(ext in wanted)
        self.log("Browsergame-Standard-Dateitypen aktiviert.")

    def enable_web_extensions(self):
        wanted = {
            ".html", ".htm", ".shtml", ".php", ".php3", ".php4", ".php5",
            ".asp", ".aspx", ".jsp", ".cgi", ".js", ".mjs", ".css", ".scss", ".less", ".json", ".xml"
        }
        for ext, var in self.ext_vars.items():
            var.set(ext in wanted)
        self.log("Web/HTML-Dateitypen aktiviert.")

    def enable_media_extensions(self):
        wanted = {
            ".png", ".jpg", ".jpeg", ".gif", ".svg", ".webp", ".bmp", ".ico", ".tga", ".dds",
            ".mp3", ".ogg", ".wav", ".flac", ".aac", ".m4a", ".mid", ".midi",
            ".mp4", ".webm", ".avi", ".mkv", ".mov", ".flv"
        }
        for ext, var in self.ext_vars.items():
            var.set(ext in wanted)
        self.log("Medien-Dateitypen aktiviert.")
    ...
    def build_about_tab(self, tab):
        ...
        ttk.Label(wrapper, text="v21 Layout + Dateitypen-Tab: archive, database, repair, validation, explorer, notes, favorites, tags.", foreground=self.colors["muted"], font=("Segoe UI", self.scale_value(11))).pack(anchor="w", pady=(0, 16))
    ...
    def ensure_project_folders(self, name):
        ...
        for folder in [
            ...
            "fonts", "archives", "documents",
            "game_assets",
            "xml", "json", "css", "js", "txt", "csv",
            "zip", "rar", "7z",
            "logs", "exports"
        ]:
    ...
    def select_all_results(self):
        ...
        self.log("Alle sichtbaren Treffer wurden markiert.")

    def clear_selection(self):
        if not hasattr(self, "tree"):
            return
        self.tree.selection_remove(self.tree.selection())
        self.log("Markierung aufgehoben.")

    def download_all_results(self):
        if not self.results:
            messagebox.showinfo(APP_NAME, "Keine Treffer vorhanden.")
            return
        urls = list(self.results.keys())
        self.stop_requested = False
        self.ensure_project_folders(self.current_project.get())
        self.status_var.set("Download aller Treffer läuft...")
        self.log(f"Download aller Treffer gestartet: {len(urls)} Dateien")
        threading.Thread(target=self.download_worker, args=(urls,), daemon=True).start()

    def download_visible_results(self):
        if not hasattr(self, "tree"):
            return
        urls = []
        for item in self.tree.get_children():
        ...
        if not urls:
            messagebox.showinfo(APP_NAME, "Keine sichtbaren Treffer vorhanden.")
            return
        self.stop_requested = False
        self.ensure_project_folders(self.current_project.get())
        self.status_var.set("Download sichtbarer Treffer läuft...")
        self.log(f"Download sichtbarer Treffer gestartet: {len(urls)} Dateien")
        threading.Thread(target=self.download_worker, args=(urls,), daemon=True).start()

    def download_by_type(self, file_type):
        urls = [
            url for url, info in self.results.items()
            if str(info.get("type", "")).lower() == file_type.lower()
        ]
        if not urls:
            messagebox.showinfo(APP_NAME, f"Keine Treffer vom Typ {file_type.upper()} vorhanden.")
            return
        self.stop_requested = False
        self.ensure_project_folders(self.current_project.get())
        self.status_var.set(f"Download {file_type.upper()} läuft...")
        self.log(f"Download {file_type.upper()} gestartet: {len(urls)} Dateien")
        threading.Thread(target=self.download_worker, args=(urls,), daemon=True).start()
